...
from research_agent.constant import API_BASE_URL, COMPLETION_MODEL
...
def visualizer(
    env: RequestsMarkdownBrowser, image_path: str, question: Optional[str] = None
) -> Result:
    """
...
{mathnote}
        """
        msg = [{"role": "user", "content": [{"type": "text", "text": wrap_ques}]}]
...
if __name__ == "__main__":
    ...
    print(
        open_local_file(
            env, f"/{workplace_name}/denoising_diffusion_implicit_models.tex"
        )
    )
    ...
    print(
        question_answer_on_whole_page(
            env,
            "Please give me the formula of the ddim model. And compare it with the ddpm model mathematically.",
        )
    )
