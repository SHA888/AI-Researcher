...
def download_arxiv_source(arxiv_url, arxiv_title):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with open(
                f"arxiv_source/{arxiv_title.replace(' ', '_').lower()}.tar.gz", "wb"
            ) as file:
                file.write(response.content)
            try:
                tex_content = extract_tex_content(
                    f"arxiv_source/{arxiv_title.replace(' ', '_').lower()}.tar.gz"
                )
                os.makedirs("arxiv_papers", exist_ok=True)
                with open(
                    f"arxiv_papers/{arxiv_title.replace(' ', '_').lower()}.tex", "w"
                ) as file:
            ...
            return (
                f"Download source failed with HTTP status code {response.status_code}"
            )
