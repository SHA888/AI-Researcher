        ...
        if response.status_code == 200:
            try:
                ...
                filepath = os.path.join(
                    paper_src_dir, f"{title.replace(' ', '_').lower()}.tar.gz"
                )
                ...
                with open(
                    os.path.join(
                        paper_tex_dir, f"{title.replace(' ', '_').lower()}.tex"
                    ),
                    "w",
                ) as f:
                    f.write(tex_content)
                return {
                    "status": 0,
                    "message": f"Download paper '{title}' successfully",
                    "path": f"/{workplace_name}/papers/{title.replace(' ', '_').lower()}.tex",
                }
            except Exception as e:
                return {
                    "status": -1,
...
def download_arxiv_source_by_title(
    paper_list: List[str], local_root: str, workplace_name: str
):
    """
    ...
    for title in paper_list:
        ...
        download_info = download_arxiv_source(
            paper["url"], local_root, workplace_name, title
        )
