...
def agent(model: str, agent_func: str, query: str, context_variables):
    ...
    agent_module = importlib.import_module("metachain.agents")
    try:
        agent_func = getattr(agent_module, agent_func)
    except AttributeError:
        raise ValueError(
            f"Agent function {agent_func} not found, you shoud check in the `metachain.agents` directory for the correct function name"
        )
