...
from research_agent.constant import AI_USER, BASE_IMAGES, GITHUB_AI_TOKEN
...
def init_container(
    ...
    if not os.path.exists(os.path.join(workplace, "tcp_server.py")):
        unzip_command = [
            "tar",
            "-xzvf",
            f"packages/{setup_package}.tar.gz",
            "-C",
            workplace,
        ]
    ...
    if git_clone:
        ...
        create_branch_command = (
            f"cd {workplace}/metachain && git checkout -b {new_branch_name}"
        )
        ...
        if result.returncode != 0:
            ...
            switch_branch_command = (
                f"cd {workplace}/metachain && git checkout {new_branch_name}"
            )
            result = subprocess.run(
    ...
    if existing_container.stdout.strip() == container_name:
        ...
        if running_container.stdout.strip() == container_name:
            print(
                f"Container '{container_name}' is already running. Skipping creation."
            )
