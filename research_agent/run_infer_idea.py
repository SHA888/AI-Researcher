...
def warp_source_papers(source_papers):
    return "\n".join(
        [
            f"Title: {source_paper['reference']}; You can use this paper in the following way: {source_paper['usage']}"
...
class InnoFlow(FlowModule):
    def __init__(
        ...
        self.code_survey_agent = AgentModule(
            get_code_survey_agent(model=CHEEP_MODEL, file_env=file_env, code_env=code_env),
            self.client,
            cache_path,
        )
        self.exp_analyser = AgentModule(
            get_exp_analyser_agent(model=CHEEP_MODEL, file_env=file_env, code_env=code_env),
            self.client,
            cache_path,
        )
    ...
    async def forward(
        ...
        **kwargs,
        ...
        data_module = importlib.import_module(
            f"benchmark.process.dataset_candidate.{category}.metaprompt"
        )
...
"""
            ...
            context_variables["experiment_report"][-1]["further_plan"]
...
def main(args, references):
    ...
    container_name = (
        args.container_name + "_" + instance_id + "_" + COMPLETION_MODEL.replace("/", "__")
    )
    ...
    flow = InnoFlow(
        cache_path="cache_" + instance_id + "_" + COMPLETION_MODEL.replace("/", "__"),
