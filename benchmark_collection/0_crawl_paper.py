...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("arxiv_search.log"), logging.StreamHandler()],
)
...
class VenueAPI:
    ...
    def get_semantic_scholar_venue(self, title: str) -> str:
        try:
            params = {"query": title, "fields": "venue,title"}
    ...
    def get_crossref_venue(self, title: str) -> str:
        try:
            params = {"query.title": title, "select": "container-title", "rows": 1}
            headers = {"User-Agent": "YourEmailAddress"}
...
async def search_arxiv(
    ...
    try:
        ...
        for result in search.results():
            should_include = (
                result.title.lower().strip() == keyword.lower().strip() if exact_match else True
            )
