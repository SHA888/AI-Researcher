...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("innovation_graph.log"), logging.StreamHandler()],
)
...
def find_pdf_file(title: str, pdf_dir: str) -> str:
    ...
    cleaned_title = " ".join(
        "".join(char if char not in string.punctuation else " " for char in title).split()
    ).lower()
...
async def process_step(
    ...
    try:
        ...
        try:
            ...
            logging.error(
                f"Failed to parse LLM response for paper: {paper['title']} (Step {step_num})"
            )
            return None
...
async def extract_model_name(
    client: GPTClient, template: str, paper_title: str, pdf_text: str
) -> str:
...
async def anonymize_text(
    client: GPTClient, template: str, model_name: str, paper_title: str, text: str
) -> str:
...
async def anonymize_paper(
    client: GPTClient,
    paper: Dict,
    pdf_text: str,
    extract_template: str,
    anonymize_template: str,
) -> Dict:
...
async def main():
    ...
    if checkpoint_path.exists():
        with open(checkpoint_path) as f:
            ...
            logging.info(
                f"Loaded {len(checkpoint_data.get('failed_papers', []))} failed papers from checkpoint"
            )
    ...
    for i, paper in enumerate(remaining_papers):
        ...
        try:
            ...
            with open(checkpoint_path, "w") as f:
                json.dump(
                    {"last_processed_index": i, "failed_papers": failed_papers},
                    f,
                    indent=2,
                )
        ...
        for paper in results:
            ...
            if pdf_path.exists():
                ...
                anonymized_paper = await anonymize_paper(
                    client, paper, pdf_text, extract_template, anonymize_template
                )
