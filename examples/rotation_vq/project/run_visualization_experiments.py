    ...
    def save_gradient_flow(self, epoch):
        ...
        for grad_info in self.gradient_history:
            self.writer.add_scalar(
                f'Gradient_Flow/{grad_info["name"]}/norm', grad_info["grad_norm"], epoch
            )
            self.writer.add_scalar(
                f'Gradient_Flow/{grad_info["name"]}/mean', grad_info["grad_mean"], epoch
            )
            self.writer.add_scalar(
                f'Gradient_Flow/{grad_info["name"]}/std', grad_info["grad_std"], epoch
            )
...
def run_visualization_experiments():
    ...
    model = VectorQuantizedVAE(
        k_dim=config["k_dim"], z_dim=config["z_dim"], beta=config["beta"]
    ).to(config["device"])
    ...
    for epoch in range(config["num_epochs"]):
        ...
        visualize_codebook_evolution(
            model, epoch, os.path.join(vis_dir, "codebook_evolution")
        )
    ...
    results = {"config": config, "training_results": training_results}
