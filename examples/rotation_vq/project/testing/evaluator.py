...
class VQVAEEvaluator:
    ...
    def compute_reconstruction_metrics(self):
        ...
        return {
            "avg_reconstruction_loss": sum(reconstruction_losses)
    ...
    def compute_sample_quality_metrics(self, num_samples=1000):
        ...
        with torch.no_grad():
            for i in range(0, num_samples, self.test_loader.batch_size):
                ...
                images = next(iter(self.test_loader))[0][
                    : min(self.test_loader.batch_size, num_samples - i)
                ]
                images = images.to(self.device)
                ...
                # Compute quality
                quality = F.mse_loss(reconstructions, images, reduction="none").mean(
                    [1, 2, 3]
                )
                reconstruction_quality.extend(quality.tolist())

        return {
            "avg_sample_quality": sum(reconstruction_quality)
            / len(reconstruction_quality)
        }
    ...
    def run_comprehensive_evaluation(self):
        ...
        return {**reconstruction_metrics, **codebook_metrics, **sample_metrics}
