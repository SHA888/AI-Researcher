...
def run_final_experiment():
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if train_metrics.get("perplexity", 0) > best_perplexity:
            ...
            torch.save(
                model.state_dict(), "results/final_experiment/best_perplexity_model.pth"
            )
        ...
        if train_metrics.get("reconstruction_loss", float("inf")) < best_recon_loss:
            ...
            torch.save(
                model.state_dict(), "results/final_experiment/best_recon_model.pth"
            )
