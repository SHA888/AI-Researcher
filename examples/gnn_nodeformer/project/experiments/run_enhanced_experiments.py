...
def run_experiment(config, dataset_name="Cora", device_id=0):
    ...
    logging.info(
        f"Adjusted number of attention heads to {adjusted_heads} for {num_features} features"
    )
    ...
    for dataset in datasets:
        ...
        if success and dataset_results:
            ...
            results[dataset] = {
                ...
                "training_stats": dataset_results[0][
                    "training_stats"
                ],  # Store first run's training stats
            }

            logging.info(f"{dataset} Results:")
            logging.info(
                f"Validation: {results[dataset]['mean_val_acc']:.4f} ± "
                f"{results[dataset]['std_val_acc']:.4f}"
            )
            logging.info(
                f"Test: {results[dataset]['mean_test_acc']:.4f} ± "
