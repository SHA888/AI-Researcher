...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        context = (
            ...
            .contiguous()
            .view(batch_size, -1, self.total_head_dim)
        )
...
class EnhancedGraphLearner(nn.Module):
    ...
    def __init__(
        ...
        self.layers.append(
            EnhancedGraphConvLayer(input_dim, hidden_dim, num_heads, dropout)
        )

        for _ in range(num_layers - 2):
            self.layers.append(
                EnhancedGraphConvLayer(hidden_dim, hidden_dim, num_heads, dropout)
            )

        self.layers.append(
            EnhancedGraphConvLayer(hidden_dim, num_classes, num_heads, dropout)
        )
        ...
        self.skip_connections = nn.ModuleList(
            [
                nn.Linear(input_dim, hidden_dim)
                if i == 0
                else nn.Linear(hidden_dim, num_classes)
                if i == num_layers - 1
                else nn.Identity()
