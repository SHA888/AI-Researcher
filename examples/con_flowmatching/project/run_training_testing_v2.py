...
from model.resnet_velocity import ImprovedCNF, ResNetVelocity
...
def train_and_evaluate(model_name, model, train_loader, test_loader, device, config):
    ...
    metrics_history = {"train_loss": [], "eval_loss": [], "fid_scores": []}
    ...
    for epoch in range(config["epochs"]):
        ...
        if (epoch + 1) % config["eval_interval"] == 0:
            metrics = evaluate_model(
                trainer.ema_model, test_loader, fid_calculator, device
            )
...
def main():
    ...
    for config_name, config in configs.items():
        train_loader, test_loader = get_data_loaders(
            data_dir, batch_size=config["batch_size"], num_workers=4
        )
        ...
        try:
            ...
            metrics_history = train_and_evaluate(
                config_name, model, train_loader, test_loader, device, config
            )

            # Save results
            results = {"config": config, "metrics": metrics_history}
