...
from model.network import CNF, VelocityNetwork
...
def main():
    ...
    baseline_metrics = evaluate_model(
        baseline_trainer.ema_model, test_loader, fid_calculator, device
    )
    logger.info(f"Baseline Model Metrics: {baseline_metrics}")

    # Evaluate improved model
    improved_metrics = evaluate_model(
        improved_trainer.ema_model, test_loader, fid_calculator, device
    )
    ...
    with open(
        f'experiments/results/comprehensive_results_{results["timestamp"]}.json', "w"
    ) as f:
