...
def run_experiments():
    ...
    for dataset in datasets:
        ...
        try:
            ...
            logging.info(
                f"{dataset} dataset loaded: {num_users} users, {num_items} items"
            )
            ...
            dataset_obj = RecommenderDataset(
                train_mat, batch_size=base_config["batch_size"]
            )
            ...
            for epoch in range(base_config["num_epochs"]):
                ...
                logging.info(
                    f"Epoch {epoch+1}/{base_config['num_epochs']} - Loss: {loss:.4f}"
                )
            ...
            logging.info(
                f"Best - Recall@20: {best_recall:.4f}, NDCG@20: {best_ndcg:.4f}"
            )
    ...
    for dataset, metrics in results.items():
        logging.info(f"\n{dataset}:")
        logging.info(
            f"Best Performance - Recall@20: {metrics['best_recall']:.4f}, NDCG@20: {metrics['best_ndcg']:.4f}"
        )
        logging.info(
            f"Final Performance - Recall@20: {metrics['final_recall']:.4f}, NDCG@20: {metrics['final_ndcg']:.4f}"
        )
