...
def run_experiment(config):
    ...
    try:
        ...
        for epoch in range(config["num_epochs"]):
            ...
            if (epoch + 1) % config["eval_frequency"] == 0:
                recall, ndcg = trainer.evaluate(
                    user_graph, user_item_graph, item_graph, test_mat, k=20
                )
                logging.info(
                    f"Epoch {epoch+1} Eval - Recall@20: {recall:.4f}, NDCG@20: {ndcg:.4f}"
                )
                ...
                if recall > best_recall:
                    ...
                    early_stop_counter = 0
                    logging.info(
                        f"New best performance - Recall@20: {recall:.4f}, NDCG@20: {ndcg:.4f}"
                    )
                else:
        ...
        logging.info(
            f"Best Results - Recall@20: {best_recall:.4f}, NDCG@20: {best_ndcg:.4f}"
        )
        ...
        plt.grid(True)
        plt.savefig(
            f'plots/loss_history_{config["temperature"]}_{config["embed_dim"]}_{config["n_layers"]}.png'
        )
        plt.close()
        ...
        eval_epochs = list(
            range(
                ...
                len(results_history["recall"]) * config["eval_frequency"] + 1,
                config["eval_frequency"],
            )
        )
        ...
        plt.grid(True)
        plt.savefig(
            f'plots/metrics_history_{config["temperature"]}_{config["embed_dim"]}_{config["n_layers"]}.png'
        )
        plt.close()
...
def run_sensitivity_analysis():
    ...
    visualize_results(
        ...
        "plots/temperature_sensitivity.png",
    )
    visualize_results(
        dim_results,
        "Embedding Dimension",
        ...
        "plots/dimension_sensitivity.png",
    )
    visualize_results(
        layer_results,
        "Number of Layers",
        "Metric Value",
        "Impact of Layer Count",
    ...
    for temp, result in temp_results.items():
        logging.info(
            f"Temperature {temp}: Recall@20={result['best_recall']:.4f}, NDCG@20={result['best_ndcg']:.4f}"
        )

    logging.info("\nEmbedding Dimension Sensitivity Results:")
    for dim, result in dim_results.items():
        logging.info(
            f"Dimension {dim}: Recall@20={result['best_recall']:.4f}, NDCG@20={result['best_ndcg']:.4f}"
        )

    logging.info("\nLayer Number Sensitivity Results:")
    for n_layers, result in layer_results.items():
        logging.info(
            f"Layers {n_layers}: Recall@20={result['best_recall']:.4f}, NDCG@20={result['best_ndcg']:.4f}"
        )
