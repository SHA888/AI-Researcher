...
class IntentGCL(nn.Module):
    def __init__(
        ...
        self.user_embedding = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(n_users, embed_dim))
        )
        self.item_embedding = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(n_items, embed_dim))
        )
    ...
    def _message_passing(self, layer_idx):
        ...
        if self.use_residual:
            self.user_embeddings[layer_idx] = (
                transformed_user + self.user_embeddings[layer_idx - 1]
            )
            self.item_embeddings[layer_idx] = (
                transformed_item + self.item_embeddings[layer_idx - 1]
            )
        else:
        ...
        # Normalization
        self.user_embeddings[layer_idx] = F.layer_norm(
            self.user_embeddings[layer_idx], self.user_embeddings[layer_idx].size()[1:]
        )
        self.item_embeddings[layer_idx] = F.layer_norm(
            self.item_embeddings[layer_idx], self.item_embeddings[layer_idx].size()[1:]
        )
