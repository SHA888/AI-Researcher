...
def plot_embeddings(user_emb, item_emb, epoch, experiment_name):
    ...
    plt.scatter(
        vis_emb[:1000, 0], vis_emb[:1000, 1], c="blue", label="Users", alpha=0.5
    )
...
def train_and_evaluate(config, experiment_name):
    ...
    adj_norm = (
        scipy_sparse_mat_to_torch_sparse_tensor(train).coalesce().to(config.device)
    )
    ...
    for epoch in range(config.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{config.epochs}"):
            ...
            optimizer.zero_grad()
            loss, loss_bpr, loss_contrast, loss_reg = model(
        ...
        with torch.no_grad():
            ...
            if epoch % 10 == 0:
                plot_embeddings(
                    model.final_user_embedding.detach().cpu().numpy(),
...
def main():
    ...
    if comparative_metrics:
        pd.DataFrame(comparative_metrics).to_csv(
            "/workplace/project/comparative_results_revision.csv"
        )
