...
# Setup logging
logging.basicConfig(
    filename="regularized_training.log",
    level=logging.INFO,
...
def main():
    ...
    criterion = RegularizedQuantizationLoss(
        beta=config["beta"], entropy_weight=config["entropy_weight"]
    )
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, images in enumerate(train_loader):
            ...
            model.decoder(quantized)

            # Compute losses
            loss, loss_components = criterion(
                encoded, quantized, model.quantizer.codebook
            )
