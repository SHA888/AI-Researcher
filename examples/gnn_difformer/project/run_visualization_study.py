...
def analyze_representation_evolution(model, data, device):
    ...
    evol_metrics = {"norm": [], "cosine_sim": [], "entropy": []}
    ...
    with torch.no_grad():
        ...
        evol_metrics["cosine_sim"].append(
            F.cosine_similarity(Z, Z_initial).mean().item()
        )
    ...
    for dataset_name in ["Cora", "CiteSeer"]:
        try:
            ...
            metrics = visualize_representations(
                representations, data.y.cpu().numpy(), log_dir, dataset_name
            )
